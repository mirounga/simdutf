...
//
// Adapted from haswell/avx2_base64.cpp. Base64 SIMD encode/decode is dominated
// by operations that have NO portable std::simd form: dynamic byte shuffle
// (pshufb / _mm256_shuffle_epi8), saturating subtract (_mm256_subs_epu8),
// fixed-point multishift (_mm256_mulhi_epu16 / _mm256_mullo_epi16 /
// _mm256_maddubs_epi16 / _mm256_madd_epi16), movemask (_mm256_movemask_epi8),
// lane-crossing alignr/permute and the thintable-driven byte compress. Per the
// stdsimd escape-hatch policy these all resolve to the x86 intrinsic arm, so
// the whole family is expressed with __m256i / _mm256_* intrinsics directly
// (the intrinsics headers are pulled in by stdsimd/intrinsics.h). The bridge to
// std::simd vec<uint8_t,32> via std::bit_cast is not needed here because nothing
// in this family is expressed as a portable std::simd vec; it is escape-hatch
// throughout.
//
// This file is #included from stdsimd/impl_base64.inc.cpp, INSIDE
//   namespace simdutf { namespace stdsimd { ... } }
// and inside the AVX2 target region (begin.h). The generic driver
// generic/base64.h consumes the block64 type / free functions defined here.
//
// TODO(verify): non-x86 arms are intentionally absent; the stdsimd backend is
// AVX2-tier and FORCE-only, so only the x86 escape hatch is exercised.
