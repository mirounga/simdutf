// ==== stdsimd utf16 -> utf8 conversion (adapted from haswell) ====
//
// There is no backend-agnostic generic header for the AVX2 utf16->utf8 path,
// so this file is adapted from src/haswell/avx2_convert_utf16_to_utf8.cpp.
//
// Per the stdsimd escape-hatch policy, the operations used here have no
// portable std::simd form (pshufb / movemask / packus / table-driven byte
// compression with per-128-lane shuffles), so we PRAGMATICALLY keep the x86
// AVX2 intrinsics. They compile and run under the AVX2 target region applied by
// stdsimd/begin.h (verified equivalent to haswell's). The vec<->__m256i bridge
// is not even required here because the algorithm never leaves raw __m256i.
//
