// ==== stdsimd utf32 -> utf16 conversion (adapted from haswell) ====
//
// There is no backend-agnostic generic header for the AVX2 utf32->utf16 path,
// so this file is adapted from src/haswell/avx2_convert_utf32_to_utf16.cpp.
//
// Per the stdsimd escape-hatch policy, the operations used here (testz /
// cmpeq / packus_epi32 32->16 narrowing / per-128-lane byte swap / movemask)
// have no portable std::simd form on this AVX2 tier, so we PRAGMATICALLY keep
// the x86 AVX2/SSE intrinsics. They compile and run under the AVX2 target
// region applied by stdsimd/begin.h (verified equivalent to haswell's). The
// algorithm never leaves raw __m256i/__m128i, so the vec<->__m256i bridge is
// not needed here.
//
